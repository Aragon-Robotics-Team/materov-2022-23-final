...
class GUIClass(object):
    def __init__(self, src=0):
        ...
        self.root.title('Megalodon ROV GUI 0.1.0')
        self.root.geometry(f'{screen_width}x{screen_height}')

        screenheight = int(0.5*screen_height)
        screenwidth = int(0.25*screen_width)

        screen = Canvas(self.root, height = 1000, width = 1600, bg="#fff")
        screen.grid(row = 0, column = 0, sticky = 'n' )

        #THIS IS FOR VIDEOFEED # # # # # DONT FUCK IT UP
        feed = Canvas(self.root, height = 500, width = 900, bg="#fff")
        feed.grid(row = 0, column = 1, sticky = 'n' )

        self.capture = cv2.VideoCapture(src)
        self.capture.set(cv2.CAP_PROP_BUFFERSIZE, 2)
        
        # Start frame retrieval thread
        self.thread = Thread(target=self.root.update, args=())
        ...
        #Emergency Halt & Start
        startBot = Button( text = "Enable Bot", command = lambda: navGUI.enableBot(), width = 50, height = 2, fg = 'green', bg = 'gray').place(x=.2*screen_width, y= .8*screen_height)
        stopBot = Button( text = "Disable Bot (Emergency Halt)", command = lambda: navGUI.disableBot(), width = 50, height = 2, fg = 'red', bg = "gray").place(x=.55*screen_width,y = .8*screen_height)
